...
3. **The locked test set is untouchable.** The test boundary is **`TEST_START = 2025-12-01`** (`data/loader.py`; declared 2026-06-11 after a leakage-redteam BLOCK — it can never move later). `load_bars`, `load_labels`, and `SigmaAnchor` truncate at the boundary by default; the greppable `_unlocked_full_span=True` kwarg is permitted ONLY for dataset builders, FR-5.3 trade-log scripts, and `eval/final_eval.py` (run once, Phase 5) — every new use requires leakage-redteam review. No other code path, notebook, or "sanity check" touches, summarizes, or plots the locked period.
