...
- [x] **3.2** Implement features against the load API. One function per feature, config-registered. *(Done 2026-06-12 — `data/features.py`: 28 spec features; minute features in a prefix-only-contract function; day features from completed-day frames; `FEATURES` tuple asserted against the master at build. Awaiting redteam review before Gate 3.)*
- [x] **3.3** Build the lookahead test harness (NFR-1.1): recompute every feature at bar t using data truncated at t; assert exact equality with the full-data computation, across many random (day, bar) pairs. This runs in CI on every change. *(Done — `tests/test_lookahead.py`: 60 sampled rows + 8 forced day-edge rows; minute features on physically truncated bars, day features on truncated daily frames, row features (D/pmkt/dist_pd*) recomputed from prefix + labels join, NaN-policy assertions, rule-#4 raw-column ban. Passed first run.)*
- [x] **3.4** Feature QA: distributions, NaN handling at day-open edges (several features are undefined in the first bars — define and document the policy), correlation matrix to spot redundancies. *(Done 2026-06-12 — `analysis/feature_qa.py`; QA addendum in `docs/feature-spec.md`. FR-1.3 decision: keep 1-min RV (no microstructure inflation, ratio 0.989). NaN policy verified + two spec corrections (first-universe-day NaN). Correlation clusters all by-design, pruning deferred to 4.6 with importance evidence.)*

**Leakage-redteam review of the feature pipeline (2026-06-12): BLOCK → remediated → PASS on re-review.** F1: harness blind spot — couldn't catch reads of the entry bar's own h/l/c; fixed by poisoning them to NaN in the truncated recompute; redteam re-planted its three leaks and all are now caught. F2: `rv5d_ratio`/`mom3d_atr`/`yest_range_vs_implied` had zero recompute coverage; all three added with strict-past windows. F3 (real bug the coverage gap hid): `yest_range_vs_implied` used today's session length for M₀(yesterday) — wrong by √(390/210) on 16 half-day-boundary days; fixed, master rebuilt, redteam verified every day full-span (0 mismatches). F4/F5 cosmetic. Re-review confirmed rebuild integrity (847,385 rows, 28 features, hash chain, default truncation).
- [x] **3.5** Assemble the master training table: features ⋈ labels, one row per (date, stride bar, side, grid_anchor). Persist versioned. *(Done — `master-5min.parquet`, 847,385 rows × 28 features + keys + targets; provenance-chained manifest (labels hash + spec version); `load_master()` hash-verified, TEST_START-truncated by default. Watch-item satisfied: `closest_approach` = closest_pts ÷ M(t) at assembly; raw `spot`/`strike`/`closest_pts`/`sigma_anchor` asserted absent at build AND in the harness.)*
- [x] **3.6** Attach p_mkt (the Phase 1 analytic no-touch baseline, frozen in 1.4) per row. Store as a column — it is both a candidate feature input and the evaluation benchmark. *(Done — `pmkt` column computed per row from the frozen baseline; harness recomputes it from prefix data.)*

**Trader review round (2026-06-12) addressed:** (1–2) spec self-consistency fixed (NaN cells #11/#24 → "NaN first universe day"; block counts corrected); (3) **trader decision (b): `is_opex_quarterly` added as feature #29** (spec v1.2, master rebuilt, scoped redteam PASS — flag verified day-constant and correct on all quarterly third-Fridays); (4) redteam feature-PR review confirmed done (BLOCK→PASS, two passes + scoped pass on the #29 diff); (5) `analysis/feature_qa.py` confirmed pre-boundary (default loads) and seeded-reproducible — RV decision and correlation numbers reproduce identically; (6) task boxes reconciled; (7) committed on branch. **Backlog item (scoped redteam, non-blocking): `_third_friday` doesn't shift OPEX to Thursday when the third Friday is a holiday — April 2025 monthly OPEX (Thu 2025-04-17) carries `is_opex=0`; one day affected in span, quarterlies unaffected.**
